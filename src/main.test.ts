...
test("parseLineWithOutput: mode", () => {
  ...
  expect(main.parseLineWithOutput("mode", output)).toBe(
    main.outputExtraStr + main.briefActivStr
  );
  expect(main.getMode()).toBe(false);
});
...
test("parseLineWithOutput: load_file", () => {
  ...
  expect(main.parseLineWithOutput("load_file stars.csv", output)).toBe(
    main.fileLoadedStr
  );
  expect(main.getLoadedCSV()).toStrictEqual([
...
test("parseModeWithOutput: search", () => {
  expect(main.parseLineWithOutput("search", output)).toBe(
    main.fileNotLoadedSearchStr
  );
  expect(
    main.parseLineWithOutput("search 0192i30219 aknfouhn3ujbs", output)
  ).toBe(main.fileNotLoadedSearchStr);
  expect(main.parseLineWithOutput("load_file stars.csv", output)).toBe(
    main.fileLoadedStr
  );
  expect(main.parseLineWithOutput("search", output)).toBe(main.searchUsageStr);
  expect(main.parseLineWithOutput("search 01293oi3ji021 foiu3j", output)).toBe(
    main.searchFailedStr
  );
});
